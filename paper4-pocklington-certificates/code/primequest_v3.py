#!/usr/bin/env python3
"""
PrimeQuest v3 — MULTI-CŒURS
Famille : p = 3m(m+1)+1,  m = 2^a · 3^b − 1
Preuve  : Théorème de Pocklington N-1

Optimisations :
  1. Crible restreint aux q ≡ 1 (mod 3) (~50% de travail)
  2. Parallélisation multi-cœurs via multiprocessing
  3. Zigzag depuis a_centre
  4. Checkpoint 60s
"""
...
if __name__ == '__main__':
    ...
    if cp:
        ...
        t_cumul  = 0.0
        print(f"\n  Nouvelle recherche v3")

    print(f"  PrimeQuest v3 — {DIGITS_CIBLE:,} ch. | {N_WORKERS} workers\n")
    ...
    with mp.Pool(N_WORKERS, initializer=_init_worker, initargs=(PREMIERS,)) as pool:
